...
export default function Workspace({ children }) {
  ...
  return <div className="workspace">
    <aside className="ws-sidebar">
      ...
      <div className="ws-sidebar-bottom"><div className="ws-access"><span className="ws-dot" />{roleLabels[user]} access</div><button className="ws-logout" onClick={logout}><LogOut size={18} />Sign out</button></div>
    </aside>
    <div className="ws-main">
      <header className="ws-topbar"><div><span className="ws-eyebrow">YOUR WORKSPACE</span><strong>{kitchen ? "Kitchen service" : user === "super_admin" ? "Restaurant control center" : "Restaurant operations"}</strong></div><div className="ws-profile"><span className="ws-role">{roleLabels[user]}</span><span className="ws-avatar" aria-hidden="true">{(session?.username || "R").slice(0, 1).toUpperCase()}</span><span className="ws-profile-name">{session?.username || "Your account"}</span></div></header>
